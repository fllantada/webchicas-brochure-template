...
import { deletePriceAction, updatePriceAction } from "../../actions";
import { formatPriceForLocale } from "../../helpers";
import type { PlainPrice } from "../../types";

interface Props {
  /** Lista completa de precios (incluye inactivos). Vienen pre-ordenados. */
  items: PlainPrice[];
}

/**
 * Lista master de precios — agrupada por categoría. Permite editar (link a
 * página de detalle), borrar, activar/desactivar inline. Para crear nuevos
 * usar el CTA "+ Nuevo precio" del header parent o el "+ Agregar" por categoría.
 */
export function PricesList({ items }: Props) {
  ...
  const grouped = useMemo(() => {
    const map: Record<PriceCategory, PlainPrice[]> = {
      facial: [],
      corporal: [],
      masaje: [],
      "manos-pies": [],
  ...
  function toggleActive(item: PlainPrice) {
    setBusyId(item._id);
    startTransition(async () => {
      const next = !item.active;
      const response = await updatePriceAction(item._id, {
  ...
  function onDelete(item: PlainPrice) {
    if (!confirm(`¿Borrar "${item.title.es}"? No se puede deshacer.`)) return;
    setBusyId(item._id);
    startTransition(async () => {
      const response = await deletePriceAction(item._id);
      setBusyId(null);
      if (!response.success) {
        alert(response.error ?? "Error");
        return;
        ...
        return (
          <section key={cat}>
            <header className="flex items-end justify-between border-b border-border pb-3 mb-4 gap-4">
              ...
              <a
                href={`/admin/prices/new?category=${cat}`}
